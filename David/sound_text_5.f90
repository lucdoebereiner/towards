...
module towards
  ...
  integer, parameter :: chars = 40, line = 30, page = 50, seconds = 60, samples = 48000
  ...
  real(8), parameter :: dts = 1.0_8 / (2.0_8 * 48000.0_8)
  ...
  integer, dimension(:,:,:), allocatable :: maxt, gert, ludt


  type(drawAreaW), target :: dw
  type(parameter) :: pagePar
  integer :: pageDisp = 1
  type(c_ptr) :: win
...
contains
  ...
  subroutine importTexts()
    ...
    ! call system("cp ../src/Texts/Gerhard.elm ./")
    ! call system("cp ../src/Texts/Ludvig.elm ./")
    
    print*, "reading Gerhard's"
    open(unit=11,file="./Gerhard.elm",status='old', action='read')
    do
       read(11, "(A)", advance="no", iostat=ios) at
       if (ios .gt. 0) then
          print*, "something is wrong readin gerhards text"!, c 
          exit
       else if (ios .eq. -1) then
          print*, "done reading gerhards text"!, c 
          exit
       else
          if (at .eq. '"') then
             if (.not. store) then 
         ...
         if (store) then
          ...
          ! print*, " page line char ", i, j, k, at, c, ios
       end if
    end do
    close(11)
    ...
    i = 1
    store = .false.
    print*, "reading Ludvigs's"
    open(unit=11,file="./Ludvig.elm",status='old', action='read')
    do
       read(11, "(A)", advance="no", iostat=ios) at
       if (ios .gt. 0) then
          print*, "something is wrong readin ludvigs text"!, c 
          exit
       else if (ios .eq. -1) then
          print*, "done reading ludvigs text"!, c 
          exit
       else
          if (at .eq. '"') then
             if (.not. store) then 
    ...
    do t = 1, ss*page
       ...
       do i = 1, numlayers
          ...
          do j = 1, numosc
             ...
             soundsyst(pn, sn, i,j) = soundsyst(p, s, i,j) + tmp * dts
          end do
       end do
  ...
  subroutine drawTowards (cr, dw)
    type(c_ptr) :: cr
    type(c_ptr) :: pixbuf, win
    character(len=40) :: ch

    real :: pt2px, linesep
    type(drawAreaW), pointer, intent(in) :: dw
    real(8) :: dimx, dimy, col
    
    integer :: i, j, k, idx, rr
    integer :: srep

    real(8) :: bx1, bv1, bx2, bv2, sx, sv    

    call cairo_set_source_rgba(cr, 0.5_c_double, 0.5_c_double, 0.5_c_double, 1.0_c_double)
    call cairo_paint(cr)


    call cairo_select_font_face(cr, "MonoLisa", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
  
    call cairo_set_source_rgba(cr, 0.1_c_double, 0.1_c_double, 0.1_c_double, 0.8_c_double)
    ...
    ! call cairo_move_to(cr, 20.0_c_double, 20.0_c_double)
    ! sprintf(output, "%+2.4E", 100.56456);
    ! call cairo_show_text(cr, "123456789012345678901234567890")
    ! call cairo_move_to(cr, 20.0_c_double, 40.0_c_double)
    ! call cairo_show_text(cr, "123456789012345678901234567890")
    ! call cairo_move_to(cr, 20.0_c_double, 70.0_c_double)
    ! call cairo_show_text(cr, "1")
    ! call cairo_move_to(cr, 20.0_c_double + pt2px, 70.0_c_double)
    ! ! call cairo_translate(cr, 27.0_c_double, 70.0_c_double)
    ! call cairo_show_text(cr, "2")
    ! do i = 1, 1
    i = pageDisp
    do j = 1, line
       do k = 1, chars
          call cairo_move_to(cr, 20.0_c_double + pt2px*(k-1), j*linesep*1.0_c_double)
          write (ch, "(A1)") i2c(ludt(i,j,k))
          ch(2:2) = c_null_char
          call cairo_show_text(cr, ch)
       end do
       do k = 1, chars
          call cairo_move_to(cr, 20.0_c_double + pt2px*(k-1+chars), j*linesep*1.0_c_double)
          write (ch, "(A1)") i2c(maxt(i,j,k))
          ch(2:2) = c_null_char
          call cairo_show_text(cr, ch)
       end do
       do k = 1, chars
          call cairo_move_to(cr, 20.0_c_double + pt2px*(k-1+2*chars), j*linesep*1.0_c_double)
          write (ch, "(A1)") i2c(gert(i,j,k))
          ch(2:2) = c_null_char
          call cairo_show_text(cr, ch)
       end do
    end do
    ...
    ! call cairo_set_source_rgba(cr, 1.0_c_double, 1.0_c_double, 0_c_double, 1.0_c_double)
    call cairo_set_line_width(cr, 1.5_c_double)
    call cairo_move_to(cr, 20.0_c_double + pt2px*chars, 1.0_c_double)
    call cairo_line_to(cr, 20.0_c_double + pt2px*chars, 30.5*linesep*1.0_c_double)
    call cairo_move_to(cr, 20.0_c_double + pt2px*chars*2, 1.0_c_double)
    call cairo_line_to(cr, 20.0_c_double + pt2px*chars*2, 30.5*linesep*1.0_c_double)
    call cairo_stroke(cr)


  end subroutine drawTowards
...
program digest
  ...
  if (io .eq. 0) then
     ...
     if (io .eq. 1) then
        ...
        allocate(gert(page,line,chars))
        allocate(ludt(page,line,chars))
     ...
     if (io .eq. 1) then 
       ...
       ! call initPhaseSpace(0, 128)
       
       ! call addParameter(dummyPar, "dummy", dummy, 0.0_8, 1.0_8, .FALSE.)
       ! call parameterMIDIOn(dummyPar, 21)
       ! dummyPar%verbose = .true.
       
       call addParameter(pagePar, "page", pageDisp, 1, page, .false.)
       call parameterMIDIOn(pagePar, 61)
       pagePar%verbose = .true.

       call addDrawAreaGui(dw)
       dw%doDraw => drawTowards
       dw%height = 640
       dw%width = 1120
       ...
       win = gtk_widget_get_parent_window(dw%drawArea)

       ! ! call startRattle()
       ! ! print*, "started rattle"

       ! ! call addSynth(syn, sonifmps)
